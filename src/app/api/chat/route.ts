...
export async function POST(req: Request) {
  try {
    ...
    - Maksimal member klan adalah 50 Orang
    ...
    - Web ini dibangun dengan tujuan untuk memberikan informasi real-time tentang clan AAA GANGS kepada anggota maupun calon anggota, serta untuk memberikan pengalaman interaktif melalui Sigma sebagai asisten virtual yang dapat menjawab pertanyaan seputar clan dan game Clash of Clans.
    `;
    ...
    const memberSummary = rawClanData.memberList
      .slice(0, 50) // Naikkan ke 20 besar, 14B sanggup kok!
      .map((m: any) => ({
        n: m.name,
        r: m.role,
        th: m.townHallLevel,
    ...
    const apiResponse = await client.chat.completions.create({
      model: 'mistral-medium-2505', // Ganti ke otak 14B yang baru
      messages: [
        {
          role: 'system',
          content: `
      ...
      temperature: 0.3, // Biar makin kreatif nge-roast orang
      max_tokens: 2048,
    });
