...
import { Trophy, Star, Zap, Building2, ArrowLeft } from "lucide-react";
import Link from "next/link";

// --- MAPPING ASSETS (SAMA SEPERTI MEMBERS PAGE) ---
...
export default async function SearchResultPage({ params }: { params: Promise<{ searched: string }> }) {
  const resolvedParams = await params;
  const playerTag = decodeURIComponent(resolvedParams.searched);
    ...
    return (
      <main className="min-h-screen bg-zinc-950 text-white flex items-center justify-center">
        <div className="text-center">
          <h1 className="text-4xl font-black italic uppercase mb-4">Player Not Found</h1>
          <p className="text-zinc-500 mb-8">Tag yang Anda cari tidak ditemukan dalam database.</p>
          <Link href="/search" className="inline-block px-6 py-3 bg-amber-500 hover:bg-amber-600 text-black font-semibold rounded-xl transition-all">
            Back to Search
          </Link>
        </div>
      </main>
  ...
  const generateSummary = () => {
    ...
    return `${player.name} adalah seorang pemain dengan level Town Hall ${player.townHallLevel} yang memiliki experience level ${player.expLevel}. Pemain ini berada di ${leagueName} dan telah meraih ${player.warStars} bintang pada perang klasik. Hero andalannya adalah ${heroInfo}. Pemain ini menunjukkan dedikasi yang konsisten terhadap permainan Clash Of Clans.`;
  };
  ...
  return (
    <main className="min-h-screen text-zinc-100 font-sans selection:bg-amber-500 selection:text-black">
      ...
      <section className="max-w-5xl mx-auto px-6 pt-24 pb-12 animate-slide-up">
        {/* Tombol Back */}
        <Link href="/search" className="inline-flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.3em] text-zinc-500 hover:text-amber-500 transition-colors mb-12">
...
function StatLarge({ label, value, sub, icon }: any) {
  return (
    <div className="p-4 bg-zinc-900/20 border border-zinc-800/50 rounded-2xl flex items-center gap-4">
      <div>{icon}</div>
      <div>
        <div className="text-[10px] text-zinc-500 font-black uppercase tracking-widest mb-1">{label}</div>
        <div className="text-3xl font-black italic leading-none">{value.toLocaleString()}</div>
        <div className="text-[8px] text-zinc-600 font-bold uppercase mt-1">{sub}</div>
      </div>
    </div>
  );
}
