import { Shield, Sword, Users, Trophy, Heart, Flame, Zap, Crown, UserPlus, Castle, Crosshair, Medal } from "lucide-react";
import { getClanData, getCurrentWar } from "@/lib/coc";

export default async function Home() {
  const [clan, war] = await Promise.all([getClanData(), getCurrentWar()]);

  if (!clan) return <div className="text-white text-center py-20 font-black tracking-tighter text-5xl">DATABASE ERROR... <br/>RECHECK API TOKEN!</div>;
  ...
  return (
    <main className="min-h-screen bg-[#050505] text-zinc-100 selection:bg-amber-500 selection:text-black overflow-x-hidden font-sans">
      {/* Navbar & Hero Section tetap sama seperti kode terakhirmu */}
      <nav className="p-6 flex justify-between items-center max-w-7xl mx-auto relative z-20">
        <div className="font-black text-2xl tracking-tighter italic text-amber-500 flex items-center gap-2">
          <img src={clan.badgeUrls.small} alt="Badge" className="w-8 h-8 drop-shadow-[0_0_10px_rgba(245,158,11,0.5)]" />
          {clan.name}
        </div>
        <div className="flex gap-6 text-[10px] md:text-xs font-black text-zinc-500 tracking-[0.2em]">
          <a href="/members" className="hover:text-amber-500 transition-all border-b border-transparent hover:border-amber-500 uppercase">MEMBERS</a>
          <a href="/" className="text-white border-b-2 border-amber-500 tracking-widest uppercase">HOME</a>
        </div>
      </nav>
              ...
              {highlights.map((h, i) => (
                <div key={i} className={`p-5 bg-zinc-950/40 border rounded-2xl group transition-all ${
                  ...
                  <div className="flex items-center gap-3 mb-2">
                    <div className={`p-1.5 rounded-lg ${h.type === 'Unlucky' ? 'bg-zinc-800 text-zinc-500' : 'bg-amber-500/10 text-amber-500'}`}>
                      {h.type === 'Unlucky' ? <Shield size={14}/> : <Zap size={14}/>}
                    </div>
                    <div className="text-xs font-black uppercase italic tracking-tight">{h.name}</div>
                  </div>
                  <div className={`text-[9px] font-bold uppercase tracking-tighter leading-tight ${h.type === 'Unlucky' ? 'text-zinc-500' : 'text-amber-500/70'}`}>
                    {h.type}
                  </div>
...
{/* FOOTER HOMEPAGE - UPGRADED TO MEMBER STYLE */}
